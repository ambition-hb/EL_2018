...
class VotersTopics:
    ...
    def get_Topics(self):
        ...
        for i in xrange(self.start, self.end):
            ...
            topics_url = 'https://www.zhihu.com/api/v4/members/'+str(self.userID_list[i])+'/following-topic-contributions?include=data%5B*%5D.topic.introduction&offset={0}&limit=20'
            topics_count = 0
            ...
            while 1:
                try:
                    ...
                    if r.status_code == 200:
                        ...
                        f = open('Cookies/voters_topics_cookies.txt', "r")
                        Lines = f.readlines()
                        if len(Lines) == 0:
                            ...
                            self.change_cookie()
                            with open('User/voters_topics_loseuser_' + str(self.fileNum) + '.txt','a+') as f1:
                                f1.write(str(i + 1) + '\n')
                    else:
                        self.delLogger(logger)
                        return
                    ...
                    if topics_count == 0:
                        ...
                        self.mongo.db.voters_topics.insert(data_plus)
                        break
                    else:
                        offset = 0
                        while 1:
    ...
    def copycookies(self):
        with open('Cookies/cookies.txt', "r") as f:
            Lines = f.readlines()
        with open('Cookies/voters_topics_cookies.txt','a+') as f_a:
    ...
    def change_cookie(self):
        with open('Cookies/voters_topics_cookies.txt', "r") as f:
            Lines = f.readlines()
            dict = eval(Lines[0])
            self.type = dict['type']
            self.headers['Cookie'] = dict['Cookie']
            self.headers['x-udid'] = dict['x-udid']
        with open('Cookies/voters_topics_cookies.txt', "w") as f_w:
            for line in Lines[1:]:
                f_w.write(line)
    ...
    def get_createpoint(self):
        self.file = open('CreatePoint/voters_topics_createpoint_' + str(self.fileNum) + '.txt','a+')
