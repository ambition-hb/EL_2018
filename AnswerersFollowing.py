...
from extractor import extract_answerers_following, extract_last_answerers

reload(sys)
sys.setdefaultencoding('utf8')

class AnswerersFollowing:
    ...
    def get_following(self):
        ...
        for i in xrange(self.start, self.end):
            ...
            # self.following_id_list = extract_answerers_following(self.user_id, self.user_id_list)
            ...
            while 1:
                try:
                    ...
                    if r.status_code == 200:
                        ...
                        f = open('Cookies/answerers_following_cookies.txt', "r")
                        Lines = f.readlines()
                        if len(Lines) == 0:
                            ...
                            self.change_cookie()
                            with open('User/answerers_following_loseuser_' + str(self.fileNum) + '.txt','a+') as f1:
                                f1.write(str(i + 1) + '\n')
                    else:
                        self.delLogger(logger)
                        return
                        ...
                        while 1:
                            try:
                                ...
                                if following_data['paging']['is_end']:
                                    ...
                                    # "answerer_type":self.following_type,
    ...
    def copycookies(self):
        with open('Cookies/cookies.txt', "r") as f:
            Lines = f.readlines()
        with open('Cookies/answerers_following_cookies.txt','a+') as f_a:
    ...
    def change_cookie(self):
        with open('Cookies/answerers_following_cookies.txt', "r") as f:
            Lines = f.readlines()
            dict = eval(Lines[0])
            self.type = dict['type']
            self.headers['Cookie'] = dict['Cookie']
            self.headers['x-udid'] = dict['x-udid']
        with open('Cookies/answerers_following_cookies.txt', "w") as f_w:
            for line in Lines[1:]:
                f_w.write(line)
    ...
    def get_createpoint(self):
        self.file = open('CreatePoint/answerers_following_createpoint_' + str(self.fileNum) + '.txt','a+')
