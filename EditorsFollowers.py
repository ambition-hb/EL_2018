...
from extractor import extract_editors_followers, extract_last_editors

reload(sys)
sys.setdefaultencoding('utf8')

class EditorsFollowers:
    ...
    def get_followers(self):
        ...
        for i in xrange(self.start, self.end):
            ...
            logfielname = '/log/' + dt + 'editors_' + sys._getframe().f_code.co_name + '.log'
            ...
            # self.follower_id_list = extract_editors_followers(self.user_id, self.user_id_list)
            ...
            while 1:
                try:
                    ...
                    if r.status_code == 200:
                        ...
                        f = open('Cookies/editors_followers_cookies.txt', "r")
                        Lines = f.readlines()
                        if len(Lines) == 0:
                            ...
                            self.change_cookie()
                            with open('User/editors_followers_loseuser_' + str(self.fileNum) + '.txt','a+') as f1:
                                f1.write(str(i + 1) + '\n')
                    else:
                        self.delLogger(logger)
                        return
                    ...
                    if follower_count == 0:
                        ...
                        self.mongo.db.FR_editors_followers.insert(data_plus)
                        break
                    elif self.follower_type == 0 and follower_count >= 4000:
                        logger.warning('用户关注数大于4000！')
                        self.delLogger(logger)
                        data_plus = {'user_id': self.user_id, "follower_count": follower_count}
                        self.mongo.db.FR_editors_followers.insert(data_plus)
                        break
                        ...
                        while 1:
                            try:
                                ...
                                if follower_data['paging']['is_end']:
                                    ...
                                    # "editor_type":self.follower_type,
                                    ...
                                    'user_id': self.user_id,
                                    "follower_count": follower_count,
                                    # "editor_count":self.follower_type,
                                    "follower": follower_list
                                    }
                                    self.mongo.db.FR_editors_followers.insert(data_plus)
                                    # if self.state:
    ...
    def copycookies(self):
        with open('Cookies/cookies.txt', "r") as f:
            Lines = f.readlines()
        with open('Cookies/editors_followers_cookies.txt','a+') as f_a:
    ...
    def change_cookie(self):
        with open('Cookies/editors_followers_cookies.txt', "r") as f:
            Lines = f.readlines()
            dict = eval(Lines[0])
            self.type = dict['type']
            self.headers['Cookie'] = dict['Cookie']
            self.headers['x-udid'] = dict['x-udid']
        with open('Cookies/editors_followers_cookies.txt', "w") as f_w:
            for line in Lines[1:]:
                f_w.write(line)
    ...
    def get_createpoint(self):
        self.file = open('CreatePoint/editors_followers_createpoint_' + str(self.fileNum) + '.txt','a+')
