...
from extractor import extract_answerers_info
...
class AnswerersInfo:
            ...
            try:
                ...
                if r.status_code == 404:
                    ...
                    return
                        ...
                        f = open('Cookies/answerers_info_cookies.txt', "r")
                        Lines = f.readlines()
                        if len(Lines) == 0:
    ...
    def get_userinfo(self):
        ...
        for i in xrange(self.start, self.end):
            ...
            self.mongo.db.answerers_info.insert(data_plus)
    ...
    def copycookies(self):
        with open('Cookies/cookies.txt', "r") as f:
            Lines = f.readlines()
        with open('Cookies/answerers_info_cookies.txt','a+') as f_a:
    ...
    def change_cookie(self):
        with open('Cookies/answerers_info_cookies.txt', "r") as f:
            Lines = f.readlines()
            dict = eval(Lines[0])
            self.type = dict['type']
            self.headers['Cookie'] = dict['Cookie']
            self.headers['x-udid'] = dict['x-udid']
        with open('Cookies/answerers_info_cookies.txt', "w") as f_w:
            for line in Lines[1:]:
                f_w.write(line)
    ...
    def get_createpoint(self):
        self.file = open('CreatePoint/answerers_info_createpoint_' + str(self.fileNum) + '.txt','a+')
