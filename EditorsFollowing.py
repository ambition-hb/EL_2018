...
class EditorsFollowing:
                ...
                try:
                    ...
                    if r.status_code == 200:
                        ...
                        if len(Lines) == 0:
                            ...
                            self.change_cookie()
                            with open('User/editors_following_loseuser_' + str(self.fileNum) + '.txt','a+') as f1:
                                f1.write(str(i + 1) + '\n')
                    else:
                        self.delLogger(logger)
                        return
    ...
    def copycookies(self):
        with open('Cookies/cookies.txt', "r") as f:
            Lines = f.readlines()
        with open('Cookies/editors_following_cookies.txt','a+') as f_a:
    ...
    def change_cookie(self):
        with open('Cookies/editors_following_cookies.txt', "r") as f:
            Lines = f.readlines()
            dict = eval(Lines[0])
            self.type = dict['type']
            self.headers['Cookie'] = dict['Cookie']
            self.headers['x-udid'] = dict['x-udid']
        with open('Cookies/editors_following_cookies.txt', "w") as f_w:
            for line in Lines[1:]:
                f_w.write(line)
    ...
    def get_createpoint(self):
        self.file = open('CreatePoint/editors_following_createpoint_' + str(self.fileNum) + '.txt','a+')
