...
class VotersInfo:
            ...
            try:
                ...
                if r.status_code == 404:
                    ...
                    return
                        ...
                        f = open('Cookies/voters_info_cookies.txt', "r")
                        Lines = f.readlines()
                        if len(Lines) == 0:
                            ...
                            self.change_cookie()
                            with open('User/voters_info_loseuser_' + str(self.fileNum) + '.txt','a+') as f1:
                                f1.write(str(i + 1) + '\n')
                else:
                    self.delLogger(logger)
                    sys.exit(0)
    ...
    def get_userinfo(self):
        ...
        items = self.mongo.db.voters_new.find()
        for item in items:
            self.userID_list.append(item.get('user_id'))
        self.current_proxy = get_IP()
        self.get_cookie()
        # self.voter_info = extract_voters_info()
        ...
        for i in xrange(self.start, self.end):
            ...
            # if self.voter_info.has_key(self.user_id) == 0:
            ...
            self.mongo.db.voters_info.insert(data_plus)
    ...
    def copycookies(self):
        with open('Cookies/cookies.txt', "r") as f:
            Lines = f.readlines()
        with open('Cookies/voters_info_cookies.txt','a+') as f_a:
    ...
    def change_cookie(self):
        with open('Cookies/voters_info_cookies.txt', "r") as f:
            Lines = f.readlines()
            dict = eval(Lines[0])
            self.type = dict['type']
            self.headers['Cookie'] = dict['Cookie']
            self.headers['x-udid'] = dict['x-udid']
        with open('Cookies/voters_info_cookies.txt', "w") as f_w:
            for line in Lines[1:]:
                f_w.write(line)
