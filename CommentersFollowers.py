...
from extractor import extract_commenters_followers,extract_last_commenters

reload(sys)
sys.setdefaultencoding('utf8')

class CommentersFollowers:
    ...
    def get_followers(self):
        ...
        self.current_proxy = get_IP()
        self.get_cookie()
        # self.user_id_list = extract_last_commenters()
        ...
        for i in xrange(self.start, self.end):
            ...
            logfielname = '/log/' + dt + 'commenters_' + sys._getframe().f_code.co_name + '.log'
            ...
            # self.follower_id_list = extract_commenters_followers(self.user_id, self.user_id_list)
            # if len(self.follower_id_list) == 0:
            #     self.follower_type = 0
            # else:
            ...
            while 1:
                try:
                    ...
                    if r.status_code == 200:
                        ...
                        f = open('Cookies/commenters_followers_cookies.txt', "r")
                        Lines = f.readlines()
                        if len(Lines) == 0:
                            ...
                            self.change_cookie()
                            with open('User/commenters_followers_loseuser_' + str(self.fileNum) + '.txt','a+') as f1:
                                f1.write(str(i + 1) + '\n')
                    else:
                        self.delLogger(logger)
                        return
                    ...
                    # 没有关注者的或者关注数大于10000的用户也要保存一下
                    if follower_count == 0:
                        logger.warning('用户没有关注者！')
                        self.delLogger(logger)
                        data_plus = {'user_id': self.user_id, "follower_count": follower_count}
                        self.mongo.db.FR_commenters_followers.insert(data_plus)
                        break
                    elif follower_count >= 4000:
                        ...
                        while 1:
                            try:
                                ...
                                if follower_data['paging']['is_end']:
                                    ...
                                    # "commenter_type":self.follower_type,
    ...
    def copycookies(self):
        with open('Cookies/cookies.txt', "r") as f:
            Lines = f.readlines()
        with open('Cookies/commenters_followers_cookies.txt','a+') as f_a:
    ...
    def change_cookie(self):
        with open('Cookies/commenters_followers_cookies.txt', "r") as f:
            Lines = f.readlines()
            dict = eval(Lines[0])
            self.type = dict['type']
            self.headers['Cookie'] = dict['Cookie']
            self.headers['x-udid'] = dict['x-udid']
        with open('Cookies/commenters_followers_cookies.txt', "w") as f_w:
            for line in Lines[1:]:
                f_w.write(line)
    ...
    def get_createpoint(self):
        self.file = open('CreatePoint/commenters_followers_createpoint_' + str(self.fileNum) + '.txt','a+')
