...
from extractor import extract_voters_followers, extract_last_voters

reload(sys)
sys.setdefaultencoding('utf8')

class VotersFollowers:
    ...
    def get_followers(self):
        ...
        items = self.mongo.db.voters.find()
        for item in items:
            self.userID_list.append(item.get('user_id'))
        self.current_proxy = get_IP()
        self.get_cookie()
        # self.user_id_list = extract_last_voters()
        ...
        for i in xrange(self.start, self.end):
            ...
            logfielname = '/log/' + dt + 'voters_' + sys._getframe().f_code.co_name + '.log'
            ...
            # self.follower_id_list = extract_voters_followers(self.user_id, self.user_id_list)
            ...
            while 1:
                try:
                    ...
                    if r.status_code == 200:
                        ...
                        f = open('Cookies/voters_followers_cookies.txt', "r")
                        Lines = f.readlines()
                        if len(Lines) == 0:
                            ...
                            self.change_cookie()
                            with open('User/voters_followers_loseuser_' + str(self.fileNum) + '.txt','a+') as f1:
                                f1.write(str(i + 1) + '\n')
                    else:
                        self.delLogger(logger)
                        return
                    ...
                    if follower_count == 0:
                        ...
                        self.mongo.db.FR_voters_followers.insert(data_plus)
                        break
    ...
    def copycookies(self):
        with open('Cookies/cookies.txt', "r") as f:
            Lines = f.readlines()
        with open('Cookies/voters_followers_cookies.txt','a+') as f_a:
    ...
    def change_cookie(self):
        with open('Cookies/voters_followers_cookies.txt', "r") as f:
            Lines = f.readlines()
            dict = eval(Lines[0])
            self.type = dict['type']
            self.headers['Cookie'] = dict['Cookie']
            self.headers['x-udid'] = dict['x-udid']
        with open('Cookies/voters_followers_cookies.txt', "w") as f_w:
            for line in Lines[1:]:
                f_w.write(line)
    ...
    def get_createpoint(self):
        self.file = open('CreatePoint/voters_followers_createpoint_' + str(self.fileNum) + '.txt','a+')
