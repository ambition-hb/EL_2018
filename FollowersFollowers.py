...
from extractor import extract_followers_followers, extract_last_followers

reload(sys)
sys.setdefaultencoding('utf8')

class FollowersFollowers:
    ...
    def get_followers(self):
        ...
        items = self.mongo.db.EL_question_followers_finalsample.find()
        for item in items:
        ...
        for i in xrange(self.start, self.end):
            ...
            # self.follower_id_list = extract_followers_followers(self.user_id, self.user_id_list)
            ...
            while 1:
                try:
                    ...
                    if r.status_code == 200:
                        ...
                        f = open('Cookies/followers_followers_cookies.txt', "r")
                        Lines = f.readlines()
                        if len(Lines) == 0:
                            ...
                            self.change_cookie()
                            with open('User/followers_followers_loseuser_' + str(self.fileNum) + '.txt','a+') as f1:
                                f1.write(str(i + 1) + '\n')
                    else:
                        self.delLogger(logger)
                        return
                    ...
                    if follower_count == 0:
                        ...
                        self.mongo.db.FR_followers_followers.insert(data_plus)
                        break
    ...
    def copycookies(self):
        with open('Cookies/cookies.txt', "r") as f:
            Lines = f.readlines()
        with open('Cookies/followers_followers_cookies.txt','a+') as f_a:
    ...
    def change_cookie(self):
        with open('Cookies/followers_followers_cookies.txt', "r") as f:
            Lines = f.readlines()
            dict = eval(Lines[0])
            self.type = dict['type']
            self.headers['Cookie'] = dict['Cookie']
            self.headers['x-udid'] = dict['x-udid']
        with open('Cookies/followers_followers_cookies.txt', "w") as f_w:
            for line in Lines[1:]:
                f_w.write(line)
    ...
    def get_createpoint(self):
        self.file = open('CreatePoint/followers_followers_createpoint_' + str(self.fileNum) + '.txt','a+')
