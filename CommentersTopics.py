...
class CommentersTopics:
    ...
    def get_Topics(self):
        ...
        items = self.mongo.db.commenters.find()
        for item in items:
            ...
            while 1:
                try:
                    ...
                    if r.status_code == 200:
                        ...
                        f = open('Cookies/commenters_topics_cookies.txt', "r")
                        Lines = f.readlines()
                        if len(Lines) == 0:
                            ...
                            self.change_cookie()
                            with open('User/commenters_topics_loseuser_' + str(self.fileNum) + '.txt','a+') as f1:
                                f1.write(str(i + 1) + '\n')
                    else:
                        self.delLogger(logger)
                        return
                    ...
                    if topics_count == 0:
                        ...
                        self.mongo.db.commenters_topics.insert(data_plus)
                        break
                    else:
                        offset = 0
                        while 1:
    ...
    def copycookies(self):
        with open('Cookies/cookies.txt', "r") as f:
            Lines = f.readlines()
        with open('Cookies/commenters_topics_cookies.txt','a+') as f_a:
    ...
    def change_cookie(self):
        with open('Cookies/commenters_topics_cookies.txt', "r") as f:
            Lines = f.readlines()
            dict = eval(Lines[0])
            self.type = dict['type']
            self.headers['Cookie'] = dict['Cookie']
            self.headers['x-udid'] = dict['x-udid']
        with open('Cookies/commenters_topics_cookies.txt', "w") as f_w:
            for line in Lines[1:]:
                f_w.write(line)
    ...
    def get_createpoint(self):
        self.file = open('CreatePoint/commenters_topics_createpoint_' + str(self.fileNum) + '.txt','a+')
